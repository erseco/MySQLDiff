...
class MySQLDiff {
    ...
    public function runSQLDiff() {
        ...
        foreach ($sqlList as $sql) {
            ...
            #if(!$res) {
            #    throw new Exception(mysqli_errno().': '.mysqli_error());
            #}
    ...
    protected function findDiffs() {
        ...
        foreach($this->xml->database->table_structure as $table) {
        	...
        	$desc_result = mysqli_query($this->db, 'DESCRIBE '.$tableName);
            ...
            if(mysqli_errno() == 1146) {
                ...
                # Get list of columns
                while($column = mysqli_fetch_object($desc_result)) {
                    $columns[] = $column;
                }
            }
            ...
            $indexes = array();
            $res = mysqli_query($this->db, 'SHOW INDEXES IN '.$tableName);
            if($res) {
                while($index = mysqli_fetch_object($res)) {
                    $indexes[] = $index;
                }
            }
